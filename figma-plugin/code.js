...
function serializeCanvas() {
    ...
    if (selection.length > 0) {
        ...
        rootNodes = figma.currentPage.children.filter(node => {
            ...
            return (node.x < viewport.x + viewport.width &&
    ...
    const walk = (node, depth) => {
        if (depth > MAX_DEPTH)
            return { type: node.type, name: node.name, id: node.id, note: 'Max depth reached' };
        const data = {
            id: node.id,
            type: node.type,
            name: node.name,
        ...
        if ('children' in node) {
            data.children = node.children.map((c) => walk(c, depth + 1));
        }
        return data;
    };
    return {
...
async function serializeLibraryComponents() {
    try {
        ...
        return libraries.slice(0, 50).map((c) => ({
    ...
    switch (action) {
        case 'createFrame':
            ...
            if (props.layoutAlign)
                frame.layoutAlign = props.layoutAlign; // 'STRETCH' | 'INHERIT'
            if (props.layoutGrow !== undefined)
                frame.layoutGrow = props.layoutGrow; // 0 | 1
            if (props.layoutPositioning)
                frame.layoutPositioning = props.layoutPositioning; // 'ABSOLUTE' | 'AUTO'
        ...
        case 'addRectangle':
            ...
            // Responsive Sizing
            if (props.layoutAlign)
                rect.layoutAlign = props.layoutAlign;
            if (props.layoutGrow !== undefined)
                rect.layoutGrow = props.layoutGrow;
            if (props.layoutPositioning)
                rect.layoutPositioning = props.layoutPositioning;
            // Add to parent if specified (simplified)
            const parent = figma.currentPage.selection[0] || figma.currentPage;
        ...
        case 'addInstance':
            let component = null;
            if (props.isRemote) {
                component = (await figma.importComponentByKeyAsync(props.componentId));
            }
            else {
                component = figma.getNodeById(props.componentId);
            }
            if (component) {
                ...
                if (props.properties && 'setProperties' in instance) {
                    instance.setProperties(props.properties);
                }
                ...
                if (props.variables) {
                    for (const [prop, varId] of Object.entries(props.variables)) {
                        ...
                        if (variable) {
                            ...
                            instance.setBoundVariable(prop, variable.id);
                        }
                    }
                }
                const instParent = figma.currentPage.selection[0] || figma.currentPage;
        ...
        case 'addPrototypeConnection':
            const source = figma.getNodeById(props.sourceNodeId);
            const destination = figma.getNodeById(props.destinationNodeId);
            ...
            if (nodes.length > 0) {
                for (const node of nodes) {
                    if ('setExplicitVariableModeForCollection' in node) {
                        node.setExplicitVariableModeForCollection(props.collectionId, props.modeId);
                    }
                }
            }
            break;
